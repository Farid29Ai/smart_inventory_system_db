...
<section class="admin-form-page admin-form-stock-transaction">
    <div class="admin-form-header">
        <div>
            <nav class="admin-form-breadcrumb" aria-label="breadcrumb">
                ...
                <span>/</span>
                <strong><?= __('Edit Stock Transaction') ?></strong>
            </nav>
            <div class="admin-form-title-row">
                <span class="admin-form-title-icon"><i class="bi bi-pencil-square"></i></span>
                <div>
                    <h1><?= __('Edit Stock Transaction') ?></h1>
                    <p><?= __('Update stock movement details and remarks.') ?></p>
                </div>
            </div>
        </div>
    ...
    <div class="admin-form-card">
        <?= $this->Form->create($stockTransactionEntity, ['class' => 'admin-modern-form']) ?>
            <?= $this->Form->hidden('admin_id', ['value' => $stockTransactionEntity->admin_id ?? ($currentUser['id'] ?? null)]) ?>
            ...
            <div class="admin-form-grid">
                ...
                <div class="admin-input-icon"><i class="bi bi-person-badge"></i><?= $this->Form->control('_admin_display', ['label' => __('Admin'), 'value' => $currentUser['name'] ?? __('Recorded Admin'), 'disabled' => true]) ?></div>
